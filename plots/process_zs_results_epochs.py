...
for k in range(len(folder_path_list)):
    ...
    y_values = y_values[1:]
    #take mean of 2 consecutive values
    ...
    # divide each eleemtn of x_values by 12.8
    x_values = [x/12.8 for x in x_values]
    x_values = [i for i in range(1, len(x_values)+1)]
    params_k, _ = curve_fit(func_k, x_values[:-1], f_k)
    print("Estimated params for k: ", params_k)
    # estimated_k = [params_k[0]*(x_values[i])**params_k[1] for i in range(len(x_values))]
    estimated_k = [(x_values[i])**params_k[0] for i in range(len(x_values))]
    # a, power = params

    f_k_list.append(f_k)
    estimated_k_list.append(estimated_k)
...
for k in range(len(folder_path_list)):
    ...
    pretty_print_list(f_k_list[k])
    plt.scatter(x_values_list[k][:-1], f_k_list[k], label=label_list[k], marker=marker_list[k], color = color_list[k])
    plt.plot(x_values_list[k][:-1], estimated_k_list[k][:-1], label=label_list[k]+" estimated", color = color_list[k])
...
plt.savefig('all_utilities_fk.png', bbox_inches='tight')