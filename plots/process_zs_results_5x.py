...
def func_d_dfn(x, a, half_life):#, b):
    b = 0.9
    base = 0.5
    n = 20
    x_ = x
    ...
    return a_*(((x_*n)**b) - (x_1*n)**b)

def func_d_tmars(x, a, half_life):#, b):
    b = 0.9
    base = 0.5
    x_ = x
    x_1 = x-1
...
def func_d_clip40(x, a, half_life):#, b):
    b = 0.9
    base = 0.5
    x_ = x
    x_1 = x-1
...
for k in range(len(folder_path_list)):
    ...
    try:
        ...
        import pdb;pdb.set_trace()
    pruned_result_dict = {k: v for k, v in pruned_result_dict.items()}
...
plt.subplot(1, 5, 1)
for k in range(len(folder_path_list)):
    max_val = max(x_values_list[k])
    x_vals = [x_values_list[k][i]/max_val for i in range(len(x_values_list[k]))]
    plt.plot(x_values_list[k], y_values_list[k], label=label_list[k], color=color_list[k])
    plt.scatter(x_vals[1:], y_values_original_list[k] , marker=marker_list[k], color=color_list[k], label=label_list[k])
plt.title('Error  vs Number of Samples')
plt.xlabel('Number of Samples (in M)')
plt.ylabel('ImageNet Error')
plt.legend()
