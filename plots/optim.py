...
class CustomOptimizer:
    ...
    def optimize(self):
        ...
        for i in range(self.num_iterations):
            ...
            pbar.update(1)
            pbar.set_description(f'Iteration {i}: loss: {loss.item():.4f} | a: {self.params[0].item():.4f} | b: {self.params[1].item():.4f} | c: {self.params[2].item():.4f} | d: {self.params[3].item():.4f}')
