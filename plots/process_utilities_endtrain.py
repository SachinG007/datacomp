...
for key in paths_16.keys():
    ...
    print("y vals", delta_y_vals_dict)
...
def get_params_from_data(data_name):
    ...
    #add a constraint that a > 0, b < 0, half_life > 0
    bounds = ([0.1, -10, 1], [10, 2, 10])
    initial_parameters = [0.8, -0.5, 2]
    #pass the bounds to curve_fit
    popt, _ = curve_fit(func, inputs, y_vals, p0=initial_parameters)#, bounds=bounds )
    return popt
...
for i, key in enumerate(paths.keys()):
    ...
    y_vals = 1 + np.cumsum(delta_y_vals)
    ...
    fitted = 1 + np.cumsum(fitted_deltas) 
    plt.plot(x_vals, fitted, color=colors[i])
    # legend outside to the right
    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', borderaxespad=0.)
plt.savefig("temp_bneg.png", bbox_inches='tight')
